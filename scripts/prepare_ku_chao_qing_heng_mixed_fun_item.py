...
def corrected_rows() -> list[tuple[str, float, float, str, str, str, str]]:
    ...
    return [
        ...
        ("l09", 31.28, 35.24, "Moonlight will not return to me", "Moonlight will not return to me", "月明かりは私へ帰らない", "月光不会回到我身边"),
        ("l10", 35.24, 37.18, "Tsuki wa kaeranu", "The moon will not return", "月は帰らない", "月不归来"),
        ("l11", 39.28, 41.56, "Ming yue bu gui chen bi hai", "The bright moon will not return, sunk in the blue sea", "明月は帰らず碧海に沈む", "明月不归沉碧海"),
        ("l12", 43.06, 44.98, "Bai yun chou se man Cang Wu", "White clouds fill Cangwu with sorrow", "白雲の愁色、蒼梧に満つ", "白云愁色满苍梧"),
        ("l13", 46.06, 48.50, "Moon over the blue sea", "Moon over the blue sea", "青い海の月", "碧海上的明月"),
        ("l14", 48.84, 50.20, "Shiroi kumo", "White clouds", "白い雲", "白云"),
        ("l15", 50.78, 53.78, "Kanashimi no iro", "The color of grief", "悲しみの色", "悲伤的颜色"),
        ("l16", 57.72, 60.20, "Zheng fan yi pian rao Peng Hu", "One expedition sail circles Penghu", "征帆一片、蓬壺をめぐる", "征帆一片绕蓬壶"),
        ("l17", 62.15, 64.39, "Ri ben Chao Qing ci di du", "Chao Qingheng leaves the city", "晁卿衡は帝都を辞す", "日本晁卿辞帝都"),
        ("l18", 64.89, 66.81, "Far beyond the waves", "Far beyond the waves", "波の彼方へ", "远在浪涛之外"),
        ("l19", 67.59, 69.73, "I still call your name", "I still call your name", "なお君の名を呼ぶ", "我仍呼唤你的名字"),
    ]
...
def write_media_item() -> None:
    ...
    manifest = {
        ...
        "provenance": {
            ...
            "lyricCorrection": "Same-render small ASR plus large-v3 full-mix/vocal-stem ASR cross-checks, user listening notes, and prompt lyric. Unsung final hook lines after 69.73s are omitted from active timing. The active mul track preserves intended pinyin/romaji/English where ASR is only a close phonetic drift. The English moonlight phrase after `Moon` is restored from the source lyric/listening gap, and all `Zheng fan yi pian rao Peng Hu` poem-pinyin lines keep the original Chinese poem translation.",
